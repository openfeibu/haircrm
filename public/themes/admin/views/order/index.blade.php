    ...
    layui.use(['jquery','element','table','laydate'], function(){
        ...
        table.render({
            ...
            ,cols: [[
                ...
                ,{field:'id',title:'ID', totalRowText: 'Total：',width:80,fixed: 'left'}
                ...
                ,{field:'freight_rmb',title:'{{ trans('order.label.freight') }} 元', width:120}
                ,{field:'actual_freight',title:'{{ trans('order.label.actual_freight') }} 元', width:120,edit:'text'}
                ,{field:'payment_sn',title:'{{ trans('order.label.payment_sn') }}', templet: '<div>@{{#  if(d.payment_sn){ }}<a href="@{{d.pay_url}}" target="_blank" class="layui-table-link">@{{d.payment_name}} @{{d.payment_sn}}</a>@{{#  } else { }}  @{{#  } }} </div>', width:180}
