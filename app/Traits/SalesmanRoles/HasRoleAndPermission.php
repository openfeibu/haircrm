...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('model.salesman_roles.salesman_role.model.model'),'salesman_salesman_role')->withTimestamps();
    }
    ...
    {
        $permissionModel = app(config('model.salesman_roles.salesman_permission.model.model'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[model.salesman_roles.salesman_permission.model.model] must be an instance of \Illuminate\Database\Eloquent\Model');
        }

        return $permissionModel::select(['salesman_permissions.slug', 'salesman_permissions.name'])
            ->join('salesman_permission_salesman_role', 'salesman_permission_salesman_role.salesman_permission_id', '=', 'salesman_permissions.id')
            ->join('salesman_roles', 'salesman_roles.id', '=', 'salesman_permission_salesman_role.salesman_role_id')
            ->whereIn('salesman_roles.id', $this->getRoles()->pluck('id')->toArray())
            ->orWhere('salesman_roles.level', '<', $this->level());
            //->groupBy(['salesman_permissions.slug']);
    }
    ...
    public function menus($parent_id = 0)
    {
        ...
        return $permissionModel::select(['salesman_permissions.slug', 'salesman_permissions.name','salesman_permissions.icon'])
            ->join('salesman_permission_salesman_role', 'salesman_permission_salesman_role.salesman_permission_id', '=', 'salesman_permissions.id')
            ->join('salesman_roles', 'salesman_roles.id', '=', 'salesman_permission_salesman_role.salesman_role_id')
            ->where('is_menu', 1)
    ...
    {
        return null;
        return $this->belongsToMany(config('model.salesman_roles.salesman_permission.model.model'))->withTimestamps();
    }
